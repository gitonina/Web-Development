...
const validateName=(name)=>{
 ...
 const validateDeportes = (deportes) => {
  if (!deportes || !Array.isArray(deportes)) return false;

  let selectedCount = 0;
  for (const opcion of deportes) {
    if (opcion.selected) {
      selectedCount++;
    }
  }
...
const validateForm=()=> {
  ...
  let deportes = myForm["deporte"].options;

  let selectedDeportes = Array.from(deportes).filter((opcion) => opcion.selected);
  ...
  if(!validateName(name)){
   ...
   if (!validateDeportes(selectedDeportes)) { // Pasa las opciones seleccionadas
